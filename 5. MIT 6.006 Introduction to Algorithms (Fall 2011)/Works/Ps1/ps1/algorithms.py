...
def algorithm3(problem, bestSeen = None, trace = None):
    ...
    counter3 += 1
    print counter3
    print 'rowSt colSt rowNum colNum'
    print problem
    # if it's empty, we're done 
    if problem.numRow <= 0 or problem.numCol <= 0:
        return None
    ...
    print 'best seen:', bestSeen

    # return if we can't see any better neighbors
    if neighbor == crossLoc and problem.get(crossLoc) >= problem.get(bestSeen):
