...
class PathFinder(object):
    ...
    def dijkstra(self, weight, nodes, source, destination):
        ...
        agenda.insert((0, source.state, None, source))
        while len(agenda) > 0:
            current_distance, ign, parent_node, current_node = agenda.extract_min()  # ign = ignore
            result_set[current_node] = parent_node

            if current_node is destination:  # result is found
                path = []
                while current_node is not None:
                    path.append(current_node)
                    current_node = result_set[current_node]
                return path

            children = current_node.adj
            for child in children:
                if child not in result_set:
                    new_distance = weight(current_node, child)
                    agenda.insert((current_distance + new_distance, child.state, current_node, child))
        else:
            return None
