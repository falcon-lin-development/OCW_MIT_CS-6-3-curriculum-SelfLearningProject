...
class PriorityQueue:
    ...
    def append(self, key):
        ...
        self._bottom_up_min_heapify(len(self.queue) - 1)
    ...
    def pop(self):
        ...
        self.queue.pop()
        self._top_down_min_heapify(0)
        return out

    def _bottom_up_min_heapify(self, i):
        # assert i is a leaf and all ancestors are min_heap
        # may break if i is not leaf
        # O(lg n)
        key = self.queue[i]
        node_i = i  # branch index
        parent_i = node_i // 2
        while self.queue[parent_i] > key:
            if node_i == 0:  # terminate if node is already the smallest element
                break
            self.queue[node_i] = self.queue[parent_i]
            self.queue[parent_i] = key
            node_i = parent_i
            parent_i = node_i // 2

    def _top_down_min_heapify(self, i):
        # provided left branch and right branch are min_heaps
        # O(lg n)
        if len(self.queue)-1 >= 2 * i + 1:
            left = 2 * i
            right = 2 * i + 1
        elif len(self.queue)-1 >= 2 * i:
            left = right = 2 * i
        else:  # itself is a leaf
            return

        if self.queue[i] <= self.queue[left] and self.queue[i] <= self.queue[right]:
            return  # already min heap
        elif self.queue[left] <= self.queue[right]:
            # swap left
            key = self.queue[i]
            self.queue[i] = self.queue[left]
            self.queue[left] = key
            self._top_down_min_heapify(left)
        else:
            # swap right
            key = self.queue[i]
            self.queue[i] = self.queue[right]
            self.queue[right] = key
            self._top_down_min_heapify(right)
