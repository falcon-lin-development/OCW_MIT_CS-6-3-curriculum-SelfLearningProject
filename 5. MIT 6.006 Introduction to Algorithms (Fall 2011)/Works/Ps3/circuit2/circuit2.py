...
"""
  ...
  def remove(self, key):
    ...
    has_left = bool(node.left)
    has_right = bool(node.right)

    if not has_left and not has_right:  # simple remove
      if not has_parent: # no parent => root
        self.root = None
        del node
        ...
        self._fix_RI(node.parent)
        del node
    elif has_left != has_right:  # logical xor // only one branch
      if has_parent and has_left:
        if node.parent.left is node:  # node is parent.left
        ...
        if node.parent.left is node:  # node is parent.left
          ...
          self._fix_RI(node.parent)
          del node
      elif not has_parent and has_left:
        node.left.parent = None
        self.root = node.left
        del node
      elif not has_parent and has_right:
  ...
  def _fix_RI(self, node):
    ...
    if node.is_AVL():
      ...
      if lh > rh:
        ...
        self._rr(node)
        self._fix_RI(node)
        if zigzag: self._fix_RI(node.left)
        ...
        self._lr(node)
        self._fix_RI(node)
        if zigzag: self._fix_RI(node.right)
      else:
        raise Exception("logical error\nnode:{0}\nis_AVL:{1}\nlh:{2}\nrh:{3}".format(node, node.is_AVL(), lh, rh))

  def _lr(self, node):
    """left rotate"""
    x = node
    y = node.right
    assert y is not None, "no rotation can operate on leaf\n{0}\n{1}\n{2}".format(x, y, x.parent)
    A = x.left
    B = y.left
    C = y.right
    # print x
...
class _BST_node(object):
  ...
  def __str__(self):
    out = "key:{0}, parent:{1}, left:{2}, right:{3}, size:{4}, height:{5}".format(self.key, \
                                                                                  bool(self.parent), bool(self.left),
                                                                                  bool(self.right), self.size,
                                                                                  self.height)
    return out
...
class CrossVerifier(object):
  ...
  def _events_from_layer(self, layer):
    ...
    for wire in layer.wires.values():
      if wire.is_horizontal():
        ...
        self.events.append([wire.x2,2, wire.object_id, 'drop', wire])
      else:
  ...
  def _compute_crossings(self, count_only):
    ...
    for event in self.events:
      ...
      if event_type == 'add':
        wire_dict[wire.object_id] = KeyWirePair(wire.y1, wire)
        self.index.add(KeyWirePair(wire.y1, wire))
      elif event_type == 'drop':
        self.index.remove(wire_dict[wire.object_id])
        del wire_dict[wire.object_id]
      else:
        self.trace_sweep_line(event_x)
        cross_wires = [kwp.wire for kwp in self.index.list(KeyWirePairL(wire.y1), KeyWirePairH(wire.y2))]
